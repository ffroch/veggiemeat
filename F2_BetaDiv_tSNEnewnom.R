...
#nice plots
extrapal<-c(brewer.pal(n=11,name="RdBu")[11],brewer.pal(n=11,name="PRGn")[9],brewer.pal(n=11,name="BrBG")[3])
braytsne<-tSNEdatacomb%>%
  ...
  geom_text(aes(label=sample.id),size=3,color="black")+
  ggforce::geom_mark_ellipse(expand=0.05,aes(fill=profilelong),alpha=0.3,color="white")+
...
df<-data.frame("kw"=0,"La-Le"=0, "Pro-La"=0, "Pro-Le"=0)
for (i in 0:2){
    dat<-hilldiv_m%>%
    filter(Order.q==i, method=="bycoverage")
  aov<-aov(qD~profile,data=dat)
  print(summary(aov))
  print(qqnorm(aov$residuals))
  kw<-kruskal.test(qD~profile,data=dat)
  print(kw)
  dt<-dunn.test(dat$qD,dat$profile, method="bonferroni")
  print(dt)
  df[i+1,]<-c(kw$p.value, dt$P.adjusted)
}



#compare proteobacteria with the others (combined)
hilldiv_m$combprof<-ifelse(hilldiv_m$profile=="Pro", "Pro", "LAB")
for (i in 0:2){
  dat<-hilldiv_m%>%
    filter(Order.q==i, method=="bycoverage")
  tt<-lm(qD~combprof,data=dat)
  print((tt))
  print(qqnorm(residuals(tt)))
  mwu<-wilcox.test(qD~combprof,data=dat)
  print(mwu)
  }
...
#save plot
ggsave("./04-finalRplots/Fig3profilesnewnom20240222.tiff", width=9.53,height=5.96, units="in",dpi=2000)
ggsave("./VeggieMeat/Fig3profilesnewnom20240222.png", width=9.53,height=5.96, units="in",dpi=300)
...
tSNEdatacomb%>%
  ...
  scale_color_manual(name="group", labels = c("pea-fibrous" ,"pea-minced", "soy-fibrous", "soy-minced"),
                     values=c(nicepal[1],nicepal[2],nicepal[3],nicepal[4]))+
  scale_fill_manual(name="profile",breaks=c("Lactobacillus", "Leuconostocaceae", "Proteobacteria"), labels = c(expression(italic("Latilactobacillus")) ,expression(italic("Leuconstocaceae")), "Pseudomonadota"),
                    values=extrapal)+
  facet_wrap(~method,ncol = 1,scale="free")+
  theme_bw()

#save plot
ggsave("./04-finalRplots/FigSM4newnom20240222.tiff", width=5.96,height=9.53, units="in",dpi=2000)
ggsave("./VeggieMeat/FigSM4newnom20240222.png", width=5.96,height=9.53, units="in",dpi=300)
