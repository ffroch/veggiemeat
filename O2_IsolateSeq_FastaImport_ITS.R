emptylist2<-readRDS("./06-outputfiles/SangerReadsList_ITS.RData")

#make a list of all the fasta files
filelist2<-list.files(path="./06-outputfiles/trimmedSangerfasta_ITS/",pattern="*.fa",full.names=T)
#generate namelist for the DNA strings
emptynamelist=vector(mode="list", length=length(filelist2))
j=0
for (file in filelist2) {
  j=j+1
  temporary<-str_remove(file,"./06-outputfiles/trimmedSangerfasta_ITS/")
...
writeXStringSet(seqstrimclean,"./06-outputfiles/ITS_seqstrimclean_40_15_100bp.fa")
