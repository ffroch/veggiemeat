...
#additional phylostats without 439 reads sample
phylo_object_red<-readRDS("./06-outputfiles/C_phylo_object.rds")
phylo_object_red<-subset_samples(phylo_object_red, sample.id!="C4")
phylostatsn<-data.frame("feature"=NA, "value"=NA)
phylostatsn[1,]<-c("no. of samples", length(sample_names(phylo_object_red)))
phylostatsn[2,]<-c("overall reads", sum(otu_table(phylo_object_red)))
phylostatsn[3,]<-c("min reads", min(sample_sums(phylo_object_red)))
phylostatsn[4,]<-c("max reads", max(sample_sums(phylo_object_red)))
phylostatsn[5,]<-c("median reads", median(sample_sums(phylo_object_red)))
phylostatsn[6,]<-c("no. of asvs", ntaxa(phylo_object_red))
phylostatsn[7,]<-c("no. of Genera", length(unique(data.frame(tax_table(phylo_object_red))$Genus)))
phylostatsn[8,]<-c("no. of Phyla", length(unique(data.frame(tax_table(phylo_object_red))$Phylum)))
phylostatsn[,2]<-as.numeric(phylostatsn[,2])
saveRDS(phylostatsn, file ="./06-outputfiles/C_phylostatsn.rds")
