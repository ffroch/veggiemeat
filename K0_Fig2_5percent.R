...
write.table(clno1,"./06-outputfiles/relAb_per_ASV_product_new.txt",sep="\t")#for Figure 1
...
relgenred<-relgen[relgen$x>0.05,]
...
relclassred<-relclass[relclass$x>0.05,]
...
relphylred<-relphyl[relphyl$x>0.05,]
...
pal<-c(brewer.pal(n=11,name="PiYG")[c(1,3,5)],
       ...
       brewer.pal(n=11,name="Spectral")[c(4,5)],
       "black")
...
ohnenulladd_l<-pivot_longer(ohnenulladd,2:4,names_to="rowclassifier",values_to="colorclassifier")
ohnenulladd_l$index5<-ifelse(ohnenulladd_l$rowclassifier=="pseudoprod",1,
...
xvals<-c("A6","B5","D4","A2","C3","B3","D3","D6","A1","A6","D1","A5","C5","C1","B4",
        "C8","B8","B3","B2")
...
#ggsave(paste0(outputdir,"Fig2large.tiff"), width=38.12,height=44, units="in",dpi=300)
#ggsave(paste0(outputdir,"Fig2.tiff"), width=9.53,height=11, units="in",dpi=2000)
