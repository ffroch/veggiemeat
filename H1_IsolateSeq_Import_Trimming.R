...
filelist<-intersect(list.files(path="./02-rawdata/SangerSeqData",pattern="*.ab1",full.names=T),
                    list.files(path="./02-rawdata/SangerSeqData",pattern="*27F",full.names=T))
...
for (file in filelist){
  ...
  i=i+1
  emptylist[[i]]<-trimseq
  writeFastaSR(vegprod1, outputDir= "./06-outputfiles/trimmedSangerfasta/")
  }
}
emptylist
emptylist2<-emptylist[-which(sapply(emptylist, is.null))]

saveRDS(emptylist2, file="./06-outputfiles/SangerReadsList.RData")
